...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case 0:
            ...
            switch (indexPath.row) {
                ...
                case 2:
                {
                    ...
                    NSString *type = @"未知";
                    if ([self.bill.billGoodsType isEqualToString:@"seckill"]) {
                        type = @"限时闪购";
                    }
                    else if ([self.bill.billGoodsType isEqualToString:@"flash"]) {
                        type = @"限量工厂";
                    }
                    else if ([self.bill.billGoodsType isEqualToString:@"comment"]) {
                        type = @"商店内购买";
                    }
                    else if ([self.bill.billGoodsType isEqualToString:@"luojia"]) {
                        type = @"裸价代购";
                    }
                    ...
                    break;
                case 3:
                    cell.textLabel.text = [NSString stringWithFormat:@"序列号:%@",self.bill.billTeamID];
                    break;
                case 4:
        ...
        default:
            ...
            [cell.imageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",Server_ImgHost, self.bill.billImgURLLaxtComponent]] placeholderImage:[UIImage imageNamed:@"pic_2loading.png"]];
