...
-(void)addScroll1{
    ...
    if (self.xl_buttonNames.count <=3) {
        ...
        self.scroll1.contentSize =CGSizeMake(screen_width/3*self.xl_buttonNames.count, 0);
    }
  /*  if (self.xl_buttonNames.count <=5) {
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (self.xl_buttonNames.count <=3) {
            ...
            temp.frame =CGRectMake(screen_width/3*i, 0, screen_width/3, self.xl_topHeight?self.xl_topHeight:50);
        }
 /*       if (self.xl_buttonNames.count <=5) {
            temp.frame =CGRectMake(screen_width/self.xl_buttonNames.count*i, 0, screen_width/self.xl_buttonNames.count, self.xl_topHeight?self.xl_topHeight:50);
        }else {
        ...
        if (temp.titleLabel.text) {
            [temp setBackgroundImage:self.xl_buttonImageNormal forState:UIControlStateNormal];
        }
        else {
            [temp setImage:self.xl_buttonImageNormal forState:UIControlStateNormal];
        }
        if (i == 0) {
            temp.selected =YES;
    ...
    self.view2 =[[UIImageView alloc]initWithFrame:CGRectMake(_x0,CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:10), size0.width, self.xl_sliderHeight?self.xl_sliderHeight:2)];
    if (self.xl_sliderImage) {
        self.view2.image = self.xl_sliderImage;
    }
    else {
...
-(void)addScroll2{
    ...
    self.scroll2.backgroundColor = [UIColor whiteColor];
    for (int i =0; i<self.xl_buttonNames.count; i++) {
...
-(void)changed:(UIButton *)button{
    ...
    if (self.xl_buttonNames.count <=3) {
        ...
        self.scroll2.contentOffset =CGPointMake(button.center.x*3 -screen_width/2, 0);
    }
/*    if (self.xl_buttonNames.count <=5) {
        self.scroll2.contentOffset =CGPointMake(button.center.x*self.xl_buttonNames.count -screen_width/2, 0);
    }else {
    ...
    NSLog(@"changed offset:%@",NSStringFromCGPoint(self.scroll2.contentOffset));
    self.currentPageIndex=(NSInteger)(self.scroll2.contentOffset.x/ScreenWidth);
    self.scroll2.delegate =self;
    if (self.delegate && [self.delegate respondsToSelector:@selector(xlscrollviewChangeCurrentPageIndex:)]) {
        [self.delegate xlscrollviewChangeCurrentPageIndex:self.currentPageIndex];
    }
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.xl_isMoveButton) {
        if (self.xl_buttonNames.count <=3) {
            ...
            button.transform =CGAffineTransformMakeTranslation((scrollView.contentOffset.x -button.frame.size.width*5*_x)/3/3, 0);
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/3, 0);
        }
    }else {
        if (self.xl_buttonNames.count <=3) {
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/self.xl_buttonNames.count, 0);
 ...
 /*       if (self.xl_buttonNames.count <=5) {
        ...
        */
        if (self.xl_buttonNames.count <=3) {
            ...
            x2 =scrollView.contentOffset.x/3;
        }
        if (x1 == x2) {
    ...
    */
    if (self.buttons.count >3) {
        ...
        if (_x<xAfter) {
            ...
            if (_x>=2 && _x<=self.xl_buttonNames.count-2 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -screen_width/3*1.5, 0);
                }];
            }
            if ((self.buttons.count ==4)|5 &&_x>3) {
                if ((_x ==4)|5) {
                    [UIView animateWithDuration:0.2 animations:^{
                        self.scroll1.contentOffset =CGPointMake(button.frame.size.width*(self.buttons.count==5?2:1), 0);
                    }];
                }
            }
        }else if (_x>xAfter) {
            ...
            if (_x>=3 && _x<=self.xl_buttonNames.count-2 ) {
                [UIView animateWithDuration:0.2 animations:^{
                    self.scroll1.contentOffset =CGPointMake(button.center.x -screen_width/3*3.5, 0);
                }];
            }
            if ((self.buttons.count ==4)|5 &&_x<3) {
                if ((_x ==1)|2) {
